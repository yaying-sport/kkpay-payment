...
public class ToastStrategy extends Handler implements IToastStrategy {
    ...
    @Override
    public void showLong(CharSequence text, long time) {
        if (mQueue.isEmpty() || !mQueue.contains(text)) {
            // 添加一个元素并返回true，如果队列已满，则返回false
            if (!mQueue.offer(text)) {
                // 移除队列头部元素并添加一个新的元素
...
//            sendEmptyMessageDelayed(TYPE_SHOW, DELAY_TIMEOUT);
            ...
            message.what = TYPE_SHOW;
            message.obj = time;
            sendMessageDelayed(message, DELAY_TIMEOUT);
        }
    }
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            case TYPE_SHOW:
                ...
                if (text != null) {
                    long toastDuration = getToastDuration(text) + DELAY_TIMEOUT;
                    if (time > 0) {
                        toastDuration = time + DELAY_TIMEOUT;
                    }
                    mToast.setText(text);
                    View view = mToast.getView();
                    if (view != null) {
                        view.setTag(R.id.toast_duration, time);
                    }
                    mToast.show();
                    // 等这个 Toast 显示完后再继续显示，要加上一些延迟
                    // 不然在某些手机上 Toast 可能会来不及消失就要进行显示，这样是显示不出来的
                    sendEmptyMessageDelayed(TYPE_CONTINUE, toastDuration);
                } else {
                    isShow = false;
                }
                break;
