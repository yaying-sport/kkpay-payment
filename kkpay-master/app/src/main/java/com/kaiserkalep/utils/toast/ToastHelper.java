        ...
        if (!isShow) {
            ...
            */
            ...
            try {
                ...
                // 当前已经显示
                isShow = true;
                long toastDuration = mToast.getDuration() == Toast.LENGTH_LONG ?
                        IToastStrategy.LONG_DURATION_TIMEOUT : IToastStrategy.SHORT_DURATION_TIMEOUT;
                View view = mToast.getView();
                if (view != null) {
                    Object tag = view.getTag(R.id.toast_duration);
                    if (tag != null && tag instanceof Long && (long) tag > 0) {
                        toastDuration = (long) tag;
                    }
                }
                // 添加一个移除吐司的任务
                sendEmptyMessageDelayed(0, toastDuration);
            } catch (Exception e) {
            }
        }
    }
    ...
    void cancel() {
        ...
        if (isShow) {
            ...
            try {
                ...
                if (null != mWindowHelper && null != mToast) {
