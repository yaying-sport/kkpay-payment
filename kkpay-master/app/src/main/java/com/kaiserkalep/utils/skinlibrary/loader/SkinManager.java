...
import com.kaiserkalep.utils.skinlibrary.SkinLoaderListener;
import com.kaiserkalep.utils.skinlibrary.utils.ResourcesCompat;
...
public class SkinManager implements ISkinLoader {
    ...
    private void setUpSkinFile(Context context) {
        try {
            ...
            if (isUpdate) { //清除旧版本皮肤数据
                MyDataManager.clearSkinFile(context);
            }
    ...
    public int getInteger(int resId) {
        int integer = context.getResources().getInteger(resId);
        if (mResources == null || isDefaultSkin) {
            return integer;
        }
        String resName = context.getResources().getResourceEntryName(resId);
        ...
        if (trueResId != 0) {
            integer = mResources.getInteger(trueResId);
        }
        return integer;
    }
