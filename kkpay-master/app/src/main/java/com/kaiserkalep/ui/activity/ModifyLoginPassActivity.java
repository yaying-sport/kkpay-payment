 ...
 * 修改登录密码
 */
public class ModifyLoginPassActivity extends ZZActivity implements TextWatcher, CleanEditTextView.MyFocusChangeListener {

    @BindView(R.id.et_input_pass_one)
    PasswordEditTextView etInputPassOne;
    @BindView(R.id.et_input_pass_two)
    PasswordEditTextView etInputPassTwo;
    @BindView(R.id.et_input_pass_thr)
    PasswordEditTextView etInputPassThr;
    ...
    public int getViewId() {
        return R.layout.modifyloginpassactivity;
    }

    @Override
    public void afterViews() {
        title = MyApp.getLanguageString(getContext(), R.string.mine_update_login_pass);
     ...
     * @param editText
     */
    private void initEdit(PasswordEditTextView... editText) {
        for (int i = 0; i < editText.length; i++) {
            PasswordEditTextView text = editText[i];
            if (text != null) {
    ...
    /**
     * 输入框焦点变化,失去焦点的校验
     * 顺序对应{@link #initEdit(PasswordEditTextView...)}
     *
     * @param v
     * @param hasFocus
     */
    ...
    public void onFocusChange(View v, boolean hasFocus) {
        ...
        switch (integer) {
            case ZERO://旧
                if (CommonUtils.StringNotNull(strPassOne) && !strPassOne.matches(StringConstants.matches) && strPassOne.length() < SIX) {
                    toast(MyApp.getLanguageString(this, R.string.login_edit_input_pass_error_tip));
                    return;
                }
                break;
            case ONE://新一
                if (CommonUtils.StringNotNull(strPassTwo) && !strPassTwo.matches(StringConstants.matches) && strPassTwo.length() < SIX) {
                    toast(MyApp.getLanguageString(this, R.string.login_edit_input_newpass_error_tip));
                    return;
                }
            case TWO://新二
                if (CommonUtils.StringNotNull(strPassThr) && !strPassThr.matches(StringConstants.matches) && strPassThr.length() < SIX) {
                    toast(MyApp.getLanguageString(this, R.string.login_edit_input_newpass_error_tip));
                    return;
                }
                break;
    ...
    private void checkInput() {
        checkPasswordIv();
        slUpdate.setClickable(CommonUtils.StringNotNull(strPassOne, strPassTwo, strPassThr) && strPassOne.length() >= SIX &&
                strPassTwo.length() >= SIX);
    }
    ...
    private void updataPwd() {
        new UserImpl(new ZZNetCallBack<Object>(this, Object.class) {
            ...
            public void onSuccess(Object response) {
                toast(MyApp.getLanguageString(ModifyLoginPassActivity.this, R.string.Revise_pass_success));
                finish();
            }
        ...
        }).updateMemberUpdateLoginPwd(strPassOne, strPassTwo, strPassThr);
    }
}
